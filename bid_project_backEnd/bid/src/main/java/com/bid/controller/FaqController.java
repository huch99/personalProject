...
public class FaqController {
	...
	private Long getCurrentUserId(Authentication authentication) {
		if (authentication != null && authentication.getPrincipal() instanceof UserDetails) {
			...
			return Long.parseLong(authentication.getName()); // Assuming JWT principal name is userId
		}
		
		if (authentication != null) {
            // 이 로그가 찍힌다면 authentication.getPrincipal()이 CustomUserDetails가 아니라는 의미입니다.
            log.warn("Authentication principal type: {}. Expected CustomUserDetails.", authentication.getPrincipal().getClass().getName());
        } else {
