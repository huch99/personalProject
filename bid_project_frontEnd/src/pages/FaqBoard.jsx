...
const FaqBoard = () => {
    ...
    const handleFaqClick = (faqId) => {
        // 상세 페이지는 로그인한 사용자만 볼 수 있도록, Frontend에서 한번 더 체크
        if (!isLoggedIn) {
            alert('로그인한 사용자만 게시글을 조회할 수 있습니다.');
            navigate('/login'); // 로그인 페이지로 리다이렉트
        } else {
            navigate(`/faq/${faqId}`); // 로그인했으면 상세 페이지로 이동
        }
    };
                ...
                <FaqList>
                    {faqs.map((faq) => (
                        <FaqListItem key={faq.faqId}>
                            {/* 목록 자체는 클릭하면 handleFaqClick으로 이동 */}
                            <FaqTitleLink to="#" onClick={() => handleFaqClick(faq.faqId)}>
                                {faq.title}
                            </FaqTitleLink>
                            <FaqMeta>
